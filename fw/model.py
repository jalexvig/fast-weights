            ...
            if FLAGS.model_name == 'RNN-LN-FW':
                ...
                self.A = tf.add(tf.scalar_mul(self.l, self.A),
                                tf.scalar_mul(self.e, tf.matmul(tf.transpose(
                                    self.h_s, [0, 2, 1]), self.h_s)), name='A')

                # Loop for S steps
                for _ in range(FLAGS.S):
                    self.h_s = (
                        tf.reshape(tf.matmul(self.X[:, t, :], self.W_x) + self.b_x, tf.shape(self.h_s)) +
                ...
                # Reshape h_s into h
                self.h = tf.reshape(self.h_s, [FLAGS.batch_size, FLAGS.num_hidden_units])
